<template>
    <h1>Organisation Reset Password</h1>
    ...
    <form @submit.prevent="submitReset" v-if="!otpSent">
        <input class="for-input" type="email" required name="" id="" placeholder="Email">
        <br/>
        <button class="for-button" type="submit">Confirm</button>
    </form>

    <form v-if="otpSent" @submit.prevent="submitPassword">
        <input class="for-input" type="text" required placeholder="OTP"/>
        <br/>
        <input class="for-input" type="password" required placeholder="password"/>
        <br/>
        <input class="for-input" type="password" required placeholder="confirm password"/>
        <br/>
    ...
    methods:{
        ...
        submitPassword(e){
            if(e.target[1].value===e.target[2].value){
                axios.post("https://managd-backend-server.onrender.com/resetPasswordOfOrganisation",{
                    email:this.email,
